#include "customdynamicfontsizelabel.h"
#include <QMouseEvent>
#define FONT_PRECISION (0.5)

CustomDynamicFontSizeLabel::CustomDynamicFontSizeLabel( QWidget* parent, Qt::WindowFlags f ) : QLabel( parent, f )
{
    m_GreenBorder = false;
    setIndent( 0 );
    setStyleSheet( "background-color: rgba(0,0,0,0%);" );
}

bool CustomDynamicFontSizeLabel::GetGreenBorder( )
{
    return m_GreenBorder;
}

void CustomDynamicFontSizeLabel::SetGreenBorder( bool nGreenBorder )
{
    m_GreenBorder = nGreenBorder;

    if( text( ).isEmpty( ) )
        setStyleSheet( m_GreenBorder ? "color: #ffffff; background-color: rgba(0,0,0,0%); border-style: solid; border-width: 3px; border-color: #66ff00;" : "color: #ffffff; background-color: rgba(0,0,0,0%);" );
    else
        setStyleSheet( m_GreenBorder ? "color: #ffffff; background-color: rgba(0,0,0,40%); border-style: solid; border-width: 3px; border-color: #66ff00;" : "color: #ffffff; background-color: rgba(0,0,0,40%);" );
}

void CustomDynamicFontSizeLabel::mousePressEvent( QMouseEvent* event )
{
    Q_UNUSED( event )

    if( event->button( ) == Qt::RightButton )
        emit clicked( true );
    else if( event->button( ) == Qt::LeftButton )
        emit clicked( false );
}

void CustomDynamicFontSizeLabel::paintEvent( QPaintEvent *event )
{
    QFont newFont = font( );
    float fontSize = getWidgetMaximumFontSize( this, this->text( ) );
    newFont.setPointSizeF( fontSize );
    setFont( newFont );
    QLabel::paintEvent(event);
}

float CustomDynamicFontSizeLabel::getWidgetMaximumFontSize( QWidget *widget, QString text )
{
    QFont font = widget->font( );
    const QRect widgetRect = widget->contentsRect( );
    const float widgetWidth = widgetRect.width( );
    const float widgetHeight = widgetRect.height( );
    QRectF newFontSizeRect;
    float currentSize = font.pointSizeF( );
    float step = currentSize / 2.0;

    /* If too small, increase step */
    if( step <= FONT_PRECISION )
        step = FONT_PRECISION * 4.0;

    float lastTestedSize = currentSize;
    float currentHeight = 0;
    float currentWidth = 0;

    if( text.isEmpty( ) )
        return currentSize;

    /* Only stop when step is small enough and new size is smaller than QWidget */
    while( step > FONT_PRECISION || ( currentHeight > widgetHeight ) || ( currentWidth > widgetWidth ) )
	{
        /* Keep last tested value */
        lastTestedSize = currentSize;

        /* Test label with its font */
        font.setPointSizeF( currentSize );

        /* Use font metrics to test */
        QFontMetricsF fm( font );

        /* Check if widget is QLabel */
        QLabel *label = qobject_cast<QLabel*>( widget );

        if( label )
            newFontSizeRect = fm.boundingRect( widgetRect, ( label->wordWrap( ) ? Qt::TextWordWrap : 0 ) | label->alignment( ), text );
        else
            newFontSizeRect = fm.boundingRect( widgetRect, 0, text );

        currentHeight = newFontSizeRect.height( );
        currentWidth = newFontSizeRect.width( );

        /* If new font size is too big, decrease it */
        if( ( currentHeight > widgetHeight ) || ( currentWidth > widgetWidth ) )
		{
            currentSize -= step;

            /* if step is small enough, keep it constant, so it converge to biggest font size */
            if( step > FONT_PRECISION )
                step /= 2.0;

            /* Do not allow negative size */
            if( currentSize <= 0 )
                break;
        }
        else /* If new font size is smaller than maximum possible size, increase it */
            currentSize += step;
    }

    return lastTestedSize;
}


void CustomDynamicFontSizeLabel::setText( QString text )
{
    if( text.isEmpty( ) )
        setStyleSheet( m_GreenBorder ? "color: #ffffff; background-color: rgba(0,0,0,0%); border-style: solid; border-width: 3px; border-color: #66ff00;" : "color: #ffffff; background-color: rgba(0,0,0,0%);" );
    else
        setStyleSheet( m_GreenBorder ? "color: #ffffff; background-color: rgba(0,0,0,40%); border-style: solid; border-width: 3px; border-color: #66ff00;" : "color: #ffffff; background-color: rgba(0,0,0,40%);" );

    text.replace( " + ", "\n+\n");
    QLabel::setText( text );
}

void CustomDynamicFontSizeLabel::clear( )
{
    setStyleSheet( m_GreenBorder ? "color: #ffffff; background-color: rgba(0,0,0,0%); border-style: solid; border-width: 3px; border-color: #66ff00;" : "color: #ffffff; background-color: rgba(0,0,0,0%);" );
    QLabel::clear( );
}

/* Do not give any size hint as it it changes during paintEvent */

QSize CustomDynamicFontSizeLabel::minimumSizeHint( ) const
{
    return QWidget::minimumSizeHint( );
}

QSize CustomDynamicFontSizeLabel::sizeHint( ) const
{
	return QWidget::sizeHint( );
}
