...
CGameProtocol::CGameProtocol( CUNM *nUNM )
{
    m_UNM = nUNM;
}
...
CIncomingJoinPlayer *CGameProtocol::RECEIVE_W3GS_REQJOIN( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 20 )
	{
		...
		{
            BYTEARRAY InternalIP = BYTEARRAY( data.begin( ) + static_cast<int>(Name.size( )) + 26, data.begin( ) + static_cast<int>(Name.size( )) + 30 );
...
CIncomingChatPlayer *CGameProtocol::RECEIVE_W3GS_CHAT_TO_HOST( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) )
	{
		...
		{
            BYTEARRAY ToPIDs = BYTEARRAY( data.begin( ) + static_cast<int>(i), data.begin( ) + static_cast<int>(i) + Total );
			i += Total;
			...
			else if( Flag == 32 && data.size( ) >= i + 5 )
			{
                ...
                BYTEARRAY ExtraFlags = BYTEARRAY( data.begin( ) + static_cast<int>(i), data.begin( ) + static_cast<int>(i) + 4 );
...
BYTEARRAY CGameProtocol::SEND_W3GS_SLOTINFOJOIN( unsigned char PID, BYTEARRAY port, BYTEARRAY externalIP, vector<CGameSlot> &gameSlots, uint32_t randomSeed, unsigned char gameType, unsigned char playerSlots, string logmessage, uint32_t gameID )
{
	unsigned char Zeros[] = { 0, 0, 0, 0 };
    BYTEARRAY SlotInfo = EncodeSlotInfo( gameSlots, randomSeed, gameType, playerSlots );
	BYTEARRAY packet;
	...
	if( port.size( ) == 2 && externalIP.size( ) == 4 )
	{
        ...
        packet.push_back( 0 );                                                          // packet length will be assigned later
        packet.push_back( 0 );                                                          // packet length will be assigned later
        UTIL_AppendByteArray( packet, static_cast<uint16_t>(SlotInfo.size( )), false );	// SlotInfo length
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_SLOTINFOJOIN", 7, gameID );
...
BYTEARRAY CGameProtocol::SEND_W3GS_PLAYERINFO( unsigned char PID, string name, BYTEARRAY externalIP, BYTEARRAY internalIP, string logmessage, uint32_t gameID )
{
		...
		if( name.size( ) <= 15 )
		{
			if( externalIP.size( ) == 4 )
			{
				if( internalIP.size( ) == 4 )
				{
                    ...
                    CONSOLE_Print( logmessage + " GAMEPROTO: SEND_W3GS_PLAYERINFO invalid internalIP size", 7, gameID );
			}
			else
                CONSOLE_Print( logmessage + " GAMEPROTO: SEND_W3GS_PLAYERINFO invalid externalIP size", 7, gameID );
		}
		else
            CONSOLE_Print( logmessage + " GAMEPROTO: SEND_W3GS_PLAYERINFO invalid name size", 7, gameID );
	}
	else
        CONSOLE_Print( logmessage + " GAMEPROTO: SEND_W3GS_PLAYERINFO invalid name", 7, gameID );

	return packet;
}

BYTEARRAY CGameProtocol::SEND_W3GS_PLAYERLEAVE_OTHERS( unsigned char PID, uint32_t leftCode, string logmessage, uint32_t gameID )
{
	BYTEARRAY packet;
	...
	if( PID != 255 )
	{
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_PLAYERLEAVE_OTHERS", 7, gameID );

	return packet;
}

BYTEARRAY CGameProtocol::SEND_W3GS_GAMELOADED_OTHERS( unsigned char PID, string logmessage, uint32_t gameID )
{
	BYTEARRAY packet;
	...
	if( PID != 255 )
	{
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_GAMELOADED_OTHERS", 7, gameID );

	return packet;
}

BYTEARRAY CGameProtocol::SEND_W3GS_SLOTINFO( vector<CGameSlot> &gameSlots, uint32_t randomSeed, unsigned char gameType, unsigned char playerSlots )
{
    BYTEARRAY SlotInfo = EncodeSlotInfo( gameSlots, randomSeed, gameType, playerSlots );
	BYTEARRAY packet;
	packet.push_back( W3GS_HEADER_CONSTANT );									// W3GS header constant
...
BYTEARRAY CGameProtocol::SEND_W3GS_INCOMING_ACTION( queue<CIncomingAction *> actions, uint16_t sendInterval )
{
	...
	if( !actions.empty( ) )
	{
        ...
        BYTEARRAY crc32 = UTIL_CreateByteArray( m_UNM->m_CRC->FullCRC( reinterpret_cast<unsigned char *>(const_cast<char *>(string( subpacket.begin( ), subpacket.end( ) ).c_str( ))), subpacket.size( ) ), false );
		crc32.resize( 2 );
...
BYTEARRAY CGameProtocol::SEND_W3GS_CHAT_FROM_HOST( unsigned char fromPID, BYTEARRAY toPIDs, unsigned char flag, BYTEARRAY flagExtra, string message, string logmessage, uint32_t gameID )
{
	BYTEARRAY packet;
    ...
    if( !toPIDs.empty( ) && !message.empty( ) && message.size( ) < 255 )
	{
        ...
        packet.push_back( 0 );                                          // packet length will be assigned later
        packet.push_back( 0 );                                          // packet length will be assigned later
        packet.push_back( static_cast<unsigned char>(toPIDs.size( )) );	// number of receivers
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_CHAT_FROM_HOST", 7, gameID );

	return packet;
}

BYTEARRAY CGameProtocol::SEND_W3GS_CHAT_FROM_HOST_BUFFER( unsigned char fromPID, BYTEARRAY toPIDs, unsigned char flag, BYTEARRAY flagExtra, string message, string logmessage, uint32_t gameID )
{
	BYTEARRAY packet;
...
BYTEARRAY CGameProtocol::SEND_W3GS_START_LAG( vector<CGamePlayer *> players, bool loadInGame, string logmessage, uint32_t gameID )
{
	BYTEARRAY packet;
	...
	if( NumLaggers > 0 )
	{
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: no laggers passed to SEND_W3GS_START_LAG", 7, gameID );
...
BYTEARRAY CGameProtocol::SEND_W3GS_GAMEINFO( bool TFT, unsigned char war3Version, BYTEARRAY mapGameType, BYTEARRAY mapFlags, BYTEARRAY mapWidth, BYTEARRAY mapHeight, string gameName, string hostName, uint32_t upTime, string mapPath, BYTEARRAY mapCRC, uint32_t slotsTotal, uint32_t slotsOpen, uint16_t port, uint32_t hostCounter, uint32_t entryKey, string logmessage, uint32_t gameID )
{
	unsigned char ProductID_ROC[] = { 51, 82, 65, 87 };	// "WAR3"
	unsigned char ProductID_TFT[] = { 80, 88, 51, 87 };	// "W3XP"
	unsigned char Version[] = { war3Version,  0,  0,  0 };
	...
	if( mapGameType.size( ) == 4 && mapFlags.size( ) == 4 && mapWidth.size( ) == 2 && mapHeight.size( ) == 2 && !gameName.empty( ) && !hostName.empty( ) && !mapPath.empty( ) && mapCRC.size( ) == 4 )
	{
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_GAMEINFO", 7, gameID );
...
BYTEARRAY CGameProtocol::SEND_W3GS_REFRESHGAME( uint32_t hostCounter, uint32_t playerSlots, uint32_t playersin )
{
	BYTEARRAY packet;
	packet.push_back( W3GS_HEADER_CONSTANT );			// W3GS header constant
	...
	packet.push_back( 0 );								// packet length will be assigned later
	UTIL_AppendByteArray( packet, hostCounter, false );	// Host Counter
	UTIL_AppendByteArray( packet, playersin, false );	// Players
...
BYTEARRAY CGameProtocol::SEND_W3GS_MAPCHECK( string mapPath, BYTEARRAY mapSize, BYTEARRAY mapInfo, BYTEARRAY mapCRC, BYTEARRAY mapSHA1, string logmessage, uint32_t gameID )
{
    unsigned char Unknown[] = { 1, 0, 0, 0 };
	BYTEARRAY packet;
	...
	if( !mapPath.empty( ) && mapSize.size( ) == 4 && mapInfo.size( ) == 4 && mapCRC.size( ) == 4 && mapSHA1.size( ) == 20 )
	{
        ...
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_MAPCHECK", 7, gameID );
...
BYTEARRAY CGameProtocol::SEND_W3GS_MAPPART( unsigned char fromPID, unsigned char toPID, uint32_t start, string *mapData, string logmessage, uint32_t gameID )
{
    unsigned char Unknown[] = { 1, 0, 0, 0 };
	BYTEARRAY packet;
	...
	if( start < mapData->size( ) )
	{
        ...
        BYTEARRAY crc32 = UTIL_CreateByteArray( m_UNM->m_CRC->FullCRC( reinterpret_cast<unsigned char *>(const_cast<char *>(mapData->c_str( ))) + start, End - start ), false );
		UTIL_AppendByteArrayFast( packet, crc32 );

		// map data

        BYTEARRAY Data = UTIL_CreateByteArray( reinterpret_cast<unsigned char *>(const_cast<char *>(mapData->c_str( ))) + start, End - start );
		UTIL_AppendByteArrayFast( packet, Data );
		AssignLength( packet );
	}
	else
        CONSOLE_Print( logmessage + " GAMEPROTO: invalid parameters passed to SEND_W3GS_MAPPART", 7, gameID );
...
BYTEARRAY CGameProtocol::EncodeSlotInfo( vector<CGameSlot> &gameSlots, uint32_t randomSeed, unsigned char gameType, unsigned char playerSlots )
{
	BYTEARRAY SlotInfo;
    SlotInfo.push_back( static_cast<unsigned char>(gameSlots.size( )) );    // number of slots

    for( uint32_t i = 0; i < gameSlots.size( ); i++ )
        UTIL_AppendByteArray( SlotInfo, gameSlots[i].GetByteArray( ) );
