...
void disjoint_rotate( Iterator begin1, size_t size1, Iterator begin2, size_t size2, Value *type )
{
...
bool next_combination( Iterator begin, Iterator mid, Iterator end )
{
	...
	std::iter_swap( head_pos, tail_pos );
    disjoint_rotate( head_pos + 1, head_len - 1, tail_pos + 1, tail_len - 1, &*head_pos );
	return true;
}
