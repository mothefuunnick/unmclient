...
public:
	CMap( CUNM *nUNM );
    CMap( CUNM *nUNM, CConfig *CFG, std::string nCFGFile, std::string nFilePath );
	~CMap( );

    void SetLogAll( bool nLogAll )
    {
        m_LogAll = nLogAll;
    }
    ...
    std::string GetMapMatchMakingCategory( )
	{
		return m_MapMatchMakingCategory;
	}
    std::string GetMapStatsW3MMDCategory( )
	{
		return m_MapStatsW3MMDCategory;
	}
    std::string GetMapDefaultHCL( )
	{
	...
	uint32_t GetMapOnlyAutoWarnIfMoreThanXPlayers( )
	{
		return m_MapOnlyAutoWarnIfMoreThanXPlayers;
	}
    uint32_t GetAutoWarnMarks( )
	{
		return m_AutoWarnMarks;
	}
    int64_t GetMapFileSize( )
    {
        return m_MapFileSize;
    }
    std::vector<CGameSlot> GetSlots( )
	{
		return m_Slots;
	}

    void Load( CConfig *CFG, std::string nCFGFile, std::string nFilePath );
	void CheckValid( );
