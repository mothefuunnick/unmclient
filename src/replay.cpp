...
void CReplay::AddLeaveGame( uint32_t reason, unsigned char PID, uint32_t result )
{
    ...
    UTIL_AppendByteArray( Block, static_cast<uint32_t>(1), false );
...
void CReplay::AddTimeSlot2( queue<CIncomingAction *> actions )
{
    ...
    UTIL_AppendByteArray( Block, static_cast<uint16_t>(0), false );
    UTIL_AppendByteArray( Block, static_cast<uint16_t>(0), false );
	...
	while( !actions.empty( ) )
	{
        ...
        UTIL_AppendByteArray( Block, static_cast<uint16_t>(Action->GetAction( )->size( )), false );
		UTIL_AppendByteArrayFast( Block, *Action->GetAction( ) );
	}

	// assign length

    BYTEARRAY LengthBytes = UTIL_CreateByteArray( static_cast<uint16_t>( Block.size( ) - 3 ), false );
...
void CReplay::AddChatMessage( unsigned char PID, unsigned char flags, uint32_t chatMode, string message )
{
    ...
    BYTEARRAY LengthBytes = UTIL_CreateByteArray( static_cast<uint16_t>( Block.size( ) - 4 ), false );
...
void CReplay::BuildReplay( string gameName, string statString, uint32_t war3Version, uint16_t buildNumber )
{
    ...
    UTIL_AppendByteArray( Replay, static_cast<uint32_t>(m_Slots.size( )), false );	// PlayerCount (4.6)
	...
	for( vector<PIDPlayer> ::iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
	{
		if( ( *i ).first != m_HostPID )
		{
            ...
            UTIL_AppendByteArray( Replay, static_cast<uint32_t>(0), false );		// Unknown
		}
	}
    ...
    UTIL_AppendByteArray( Replay, static_cast<uint16_t>( 7 + m_Slots.size( ) * 9 ), false );    // Size (4.10)
	Replay.push_back( static_cast<unsigned char>( m_Slots.size( ) ) );							// NumSlots (4.10)
    ...
    UTIL_AppendByteArray( Replay, static_cast<uint32_t>(1), false );
...
#define READB( x, y, z )	(x).read( reinterpret_cast<char *>(y), (z) )
...
void CReplay::ParseReplay( bool parseBlocks )
{
	...
	while( 1 )
	{
		...
		else if( Garbage1 == CReplay::REPLAY_TIMESLOT )
		{
			...
			if( BlockSize >= 2 )
                ActualReplayLength += GarbageData[0] | static_cast<unsigned int>(GarbageData[1]) << 8;
