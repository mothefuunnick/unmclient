...
public:
    ...
    std::vector<PIDPlayer> GetPlayers( )
	{
		return m_Players;
	}
    ...
    std::queue<BYTEARRAY> *GetLoadingBlocks( )
	{
		return &m_LoadingBlocks;
	}
    std::queue<BYTEARRAY> *GetBlocks( )
	{
		return &m_Blocks;
	}
    ...
    void AddPlayer( unsigned char nPID, std::string nName )
	{
		m_Players.push_back( PIDPlayer( nPID, nName ) );
	}
    void SetSlots( std::vector<CGameSlot> nSlots )
	{
		m_Slots = nSlots;
	}
    ...
    void SetHostName( std::string nHostName )
	{
