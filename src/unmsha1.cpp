...
void CSHA1::Final( )
{
	...
	for( i = 0; i < 8; i++ )
        finalcount[i] = static_cast<unsigned char>( ( m_count[( i >= 4 ? 0 : 1 )] >> ( ( 3 - ( i & 3 ) ) * 8 ) ) & 255 ); // Endian independent

    Update( reinterpret_cast<unsigned char *>(const_cast<char *>("\200")), 1 );

	while( ( m_count[0] & 504 ) != 448 )
        Update( reinterpret_cast<unsigned char *>(const_cast<char *>("\0")), 1 );
