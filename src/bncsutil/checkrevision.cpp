...
extern "C" {
	...
	MEXP( long ) get_mpq_seed( int mpq_number )
	{
		...
		return checkrevision_seeds[(unsigned int)mpq_number];
	}
	...
	MEXP( long ) set_mpq_seed( int mpq_number, long new_seed )
	{
		...
		{
			ret = checkrevision_seeds[(unsigned int)mpq_number];
		}

		checkrevision_seeds[(unsigned int)mpq_number] = new_seed;
		return ret;
	}
							   ...
							   int mpqNumber, unsigned long* checksum )
	{
		...
		while( token && *token )
		{
			if( *( token + 1 ) == '=' )
			{
				...
				if( BUCR_ISNUM( *token ) )
				{
					values[variable] = (uint64_t)ATOL64( token );
				}
				else
				{
					if( curFormula > 3 )
		...
		values[0] ^= checkrevision_seeds[(unsigned int)mpqNumber];
