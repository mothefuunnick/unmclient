...
#include <bncsutil/bncssha1.h> // US Secure Hash Algorithm (for W3)
...
{
	if( w3value2 != nullptr && keyType == KEY_WARCRAFT3 )
	{
...
int CDKeyDecoder::processWarCraft2Key( )
{
	...
	for( i = 0; i < 16; i += 2 )
	{
		...
		n2 = n >> 4;
		// !
		cdkey[i] = getHexValue( (int)n2 );
		cdkey[i + 1] = getHexValue( (int)n );
		r <<= 1;
	}
	...
	for( i = 0; i < 16; i++ )
	{
		c = (unsigned char)cdkey[i];
		n = (unsigned long)getNumValue( (char)c );
		n2 = v * 2;
		n ^= n2;
		v += n;
	}
	...
	for( int j = 15; j >= 0; j-- )
	{
		...
		{
			n = (unsigned long)( 0xF - ( 8 - j ) );
		}
		n &= 0xF;
		c2 = (unsigned char)cdkey[n];
		cdkey[j] = (char)c2;
		cdkey[n] = (char)c;
	}
	v2 = 0x13AC9741;
	for( int j = 15; j >= 0; j-- )
	{
		c = (unsigned char)toupper( cdkey[j] );
		cdkey[j] = (char)c;
		if( c <= '7' )
		{
			v = v2;
			c2 = (unsigned char)( ( (char)( v & 0xFF ) ) & 7 ^ c );
			v >>= 3;
			cdkey[j] = (char)c2;
			v2 = v;
		}
...
int CDKeyDecoder::processWarCraft3Key( )
{
	...
	{
		cdkey[i] = (char)toupper( cdkey[i] );
		a = ( b + 0x07B5 ) % W3_BUFLEN;
		b = ( a + 0x07B5 ) % W3_BUFLEN;
		decode = (char)( w3KeyMap[(int)cdkey[i]] );
		table[a] = ( decode / 5 );
		table[b] = ( decode % 5 );
	}
	...
	product = (unsigned long)( values[0] >> 0xA );
	product = SWAP4( product );
#if LITTLEENDIAN
	for( i = 0; i < 4; i++ )
	{
		values[i] = (int)MSB4( values[i] );
	}
#endif
...
void CDKeyDecoder::decodeKeyTable( int* keyTable )
{
	...
	do
	{
		...
		esi = ( var8 & 7 ) << 2;
		var4 = var8 >> 3;
		varC = (unsigned int)keyTable[3 - var4];
		varC &= ( 0xF << esi );
		varC = varC >> esi;
		...
		if( i < 464 )
		{
			...
			{
				ecx = (unsigned int)( ( j & 7 ) << 2 );
				ebp = (unsigned int)( keyTable[0x3 - ( j >> 3 )] );
				ebp &= ( 0xF << ecx );
				ebp = ebp >> ecx;
		...
		j = (int)( 3 - var4 );
		ebx = (unsigned int)( ( w3TranslateMap[varC + i] & 0xF ) << esi );
		keyTable[j] = (int)( ebx | ~( 0xF << esi ) & ( (int)keyTable[j] ) );
	}
	while( ( i -= 16 ) >= 0 );
	...
	for( i = 0; i < 4; i++ )
	{
		copy[i] = (unsigned int)LSB4( keyTable[i] );
	}
	...
	for( edi = 0; edi < 120; edi++ )
	{
		...
		location -= ( ( esi >> 5 ) << 2 );
		ebp = (unsigned int)( *(int*)( scopy + location ) );
		ebp = LSB4( ebp );
		ebp &= ( 1 << ecx );
		ebp = ebp >> ecx;
		ckt = ( keyTable + edx );
