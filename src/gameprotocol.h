...
public:
    ...
    BYTEARRAY SEND_W3GS_SLOTINFOJOIN( unsigned char PID, BYTEARRAY port, BYTEARRAY externalIP, std::vector<CGameSlot> &gameSlots, uint32_t randomSeed, unsigned char gameType, unsigned char playerSlots, std::string logmessage, uint32_t gameID );
	BYTEARRAY SEND_W3GS_REJECTJOIN( uint32_t reason );
    BYTEARRAY SEND_W3GS_PLAYERINFO( unsigned char PID, std::string name, BYTEARRAY externalIP, BYTEARRAY internalIP, std::string logmessage, uint32_t gameID );
    BYTEARRAY SEND_W3GS_PLAYERLEAVE_OTHERS( unsigned char PID, uint32_t leftCode, std::string logmessage, uint32_t gameID );
    BYTEARRAY SEND_W3GS_GAMELOADED_OTHERS( unsigned char PID, std::string logmessage, uint32_t gameID );
    BYTEARRAY SEND_W3GS_SLOTINFO( std::vector<CGameSlot> &gameSlots, uint32_t randomSeed, unsigned char gameType, unsigned char playerSlots );
    ...
    BYTEARRAY SEND_W3GS_CHAT_FROM_HOST( unsigned char fromPID, BYTEARRAY toPIDs, unsigned char flag, BYTEARRAY flagExtra, std::string message, std::string logmessage, uint32_t gameID );
    BYTEARRAY SEND_W3GS_CHAT_FROM_HOST_BUFFER( unsigned char fromPID, BYTEARRAY toPIDs, unsigned char flag, BYTEARRAY flagExtra, std::string message, std::string logmessage, uint32_t gameID );
    BYTEARRAY SEND_W3GS_START_LAG( std::vector<CGamePlayer *> players, bool loadInGame, std::string logmessage, uint32_t gameID );
    ...
    BYTEARRAY SEND_W3GS_GAMEINFO( bool TFT, unsigned char war3Version, BYTEARRAY mapGameType, BYTEARRAY mapFlags, BYTEARRAY mapWidth, BYTEARRAY mapHeight, std::string gameName, std::string hostName, uint32_t upTime, std::string mapPath, BYTEARRAY mapCRC, uint32_t slotsTotal, uint32_t slotsOpen, uint16_t port, uint32_t hostCounter, uint32_t entryKey, std::string logmessage, uint32_t gameID );
	BYTEARRAY SEND_W3GS_CREATEGAME( bool TFT, unsigned char war3Version );
	BYTEARRAY SEND_W3GS_REFRESHGAME( uint32_t hostCounter, uint32_t playerSlots, uint32_t playersin );
	BYTEARRAY SEND_W3GS_DECREATEGAME( uint32_t hostCounter );
    BYTEARRAY SEND_W3GS_MAPCHECK( std::string mapPath, BYTEARRAY mapSize, BYTEARRAY mapInfo, BYTEARRAY mapCRC, BYTEARRAY mapSHA1, std::string logmessage, uint32_t gameID );
	BYTEARRAY SEND_W3GS_STARTDOWNLOAD( unsigned char fromPID );
    BYTEARRAY SEND_W3GS_MAPPART( unsigned char fromPID, unsigned char toPID, uint32_t start, std::string *mapData, std::string logmessage, uint32_t gameID );
...
private:
    ...
    BYTEARRAY EncodeSlotInfo( std::vector<CGameSlot> &gameSlots, uint32_t randomSeed, unsigned char gameType, unsigned char playerSlots );
};
