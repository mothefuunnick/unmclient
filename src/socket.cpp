...
string CSocket::GetIPString( )
{
	...
	return inet_ntop( AF_INET, &m_SIN.sin_addr, strIPString, sizeof( strIPString ) );
}
...
void CTCPSocket::Reset( )
{
    ...
    if( SocketLogging )
        CONSOLE_Print( "[TCPSOCKET] SOCKET RESET!" );
}
...
void CTCPSocket::DoRecv( fd_set *fd, string logmessage )
{
	...
	if( FD_ISSET( m_Socket, fd ) )
	{
		...
		if( c > 0 )
		{
            ...
            if( SocketLogging )
                CONSOLE_Print( logmessage + " TCPSOCKET: RECEIVE <<< " + UTIL_ByteArrayToHexString( UTIL_CreateByteArray( (unsigned char *)buffer, c ) ) );
        ...
        else if( c == SOCKET_ERROR && GetLastError( ) != EWOULDBLOCK )
		{
			...
			m_HasError = true;
			m_Error = GetLastError( );
            CONSOLE_Print( logmessage + " TCPSOCKET: error (recv) - " + GetErrorString( ) );
			return;
		}
		else if( c == 0 )
		{
            ...
            CONSOLE_Print( logmessage + " TCPSOCKET: closed by remote host" );
			m_Connected = false;
        }
	}
}

void CTCPSocket::DoSend( fd_set *send_fd, string logmessage )
{
	...
	if( FD_ISSET( m_Socket, send_fd ) )
	{
		...
		if( s > 0 )
		{
            ...
            if( SocketLogging )
                CONSOLE_Print( logmessage + " TCPSOCKET: SEND >>> " + UTIL_ByteArrayToHexString( BYTEARRAY( m_SendBuffer.begin( ), m_SendBuffer.begin( ) + s ) ) );
        ...
        else if( s == SOCKET_ERROR && GetLastError( ) != EWOULDBLOCK )
		{
			...
			m_HasError = true;
            m_Error = GetLastError( );
            CONSOLE_Print( logmessage + " TCPSOCKET: error (send) - " + GetErrorString( ) );
			return;
        }
	}
}
...
void CTCPClient::Connect( string localaddress, string address, uint16_t port, string logmessage )
{
	...
	if( !localaddress.empty( ) )
	{
        ...
        if( ::bind( m_Socket, ( struct sockaddr * )&LocalSIN, sizeof( LocalSIN ) ) == SOCKET_ERROR )
		{
			m_HasError = true;
			m_Error = GetLastError( );
            CONSOLE_Print( logmessage + " TCPCLIENT: error (bind) - " + GetErrorString( ) );
			return;
		}
	}
	...
	{
		m_HasError = true;
        CONSOLE_Print( logmessage + " TCPCLIENT: error (gethostbyname)" );
		return;
	}
	...
	if( connect( m_Socket, ( struct sockaddr * )&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
	{
		if( GetLastError( ) != EINPROGRESS && GetLastError( ) != EWOULDBLOCK )
		{
			...
			m_HasError = true;
			m_Error = GetLastError( );
            CONSOLE_Print( logmessage + " TCPCLIENT: error (connect) - " + GetErrorString( ) );
			return;
		}
	}
...
bool CTCPServer::Listen( string address, uint16_t port, string logmessage )
{
    ...
    else if( ( m_SIN.sin_addr.s_addr = inet_addr( address.c_str( ) ) ) == INADDR_NONE )
        m_SIN.sin_addr.s_addr = INADDR_ANY;
    ...
    if( ::bind( m_Socket, ( struct sockaddr * )&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
	{
		m_HasError = true;
        m_Error = GetLastError( );
        CONSOLE_Print( logmessage + " TCPSERVER: error (bind) - " + GetErrorString( ) );
		return false;
	}
	...
	if( listen( m_Socket, 8 ) == SOCKET_ERROR )
	{
		m_HasError = true;
        m_Error = GetLastError( );
        CONSOLE_Print( logmessage + " TCPSERVER: error (listen) - " + GetErrorString( ) );
		return false;
	}
...
bool CUDPSocket::SendTo( string address, uint16_t port, BYTEARRAY message, string cmessage )
{
	if( m_Socket == INVALID_SOCKET || m_HasError )
		return false;
	...
	if( !HostInfo )
	{
		...
		if( cmessage.empty( ) )
			...
			CONSOLE_Print( cmessage + " UDPSOCKET: error (gethostbyname)" );
...
bool CUDPSocket::Broadcast( uint16_t port, BYTEARRAY message, string logmessage )
{
	if( m_Socket == INVALID_SOCKET || m_HasError )
		return false;
	...
	if( sendto( m_Socket, MessageString.c_str( ), MessageString.size( ), 0, ( struct sockaddr * )&sin, sizeof( sin ) ) == -1 )
	{
        CONSOLE_Print( logmessage + " UDPSOCKET: failed to broadcast packet (port " + UTIL_ToString( port ) + ", size " + UTIL_ToString( MessageString.size( ) ) + " bytes)" );
		return false;
	}
...
void CUDPSocket::SetBroadcastTarget( string subnet, string logmessage )
{
	if( subnet.empty( ) )
	{
        CONSOLE_Print( logmessage + " UDPSOCKET: using default broadcast target" );
		m_BroadcastTarget.s_addr = INADDR_BROADCAST;
	}
	else
	{
        ...
        CONSOLE_Print( logmessage + " UDPSOCKET: using broadcast target [" + subnet + "]" );
		...
		{
            CONSOLE_Print( logmessage + " UDPSOCKET: invalid broadcast target, using default broadcast target" );
			m_BroadcastTarget.s_addr = INADDR_BROADCAST;
		}
	}
}
