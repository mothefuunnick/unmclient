...
#include "src/stormlib/zlib/zlib.h"
...
bool CPacked::Save( bool TFT, string fileName )
{
	...
	if( m_Valid )
	{
        ...
        return UTIL_FileWrite( fileName, reinterpret_cast<unsigned char *>(const_cast<char *>(m_Compressed.c_str( ))), m_Compressed.size( ) );
	}
	else
		return false;
}
...
bool CPacked::Extract( string inFileName, string outFileName )
{
	...
	if( m_Valid )
        return UTIL_FileWrite( outFileName, reinterpret_cast<unsigned char *>(const_cast<char *>(m_Decompressed.c_str( ))), m_Decompressed.size( ) );
	else
		return false;
}
...
bool CPacked::Pack( bool TFT, string inFileName, string outFileName )
{
	...
	if( m_Valid )
        return UTIL_FileWrite( outFileName, reinterpret_cast<unsigned char *>(const_cast<char *>(m_Compressed.c_str( ))), m_Compressed.size( ) );
	else
		return false;
}
...
void CPacked::Decompress( bool allBlocks )
{
    ...
    ISS.read( reinterpret_cast<char *>(&m_HeaderSize), 4 );			// header size
    ISS.read( reinterpret_cast<char *>(&m_CompressedSize), 4 );		// compressed file size
    ISS.read( reinterpret_cast<char *>(&m_HeaderVersion), 4 );		// header version
    ISS.read( reinterpret_cast<char *>(&m_DecompressedSize), 4 );		// decompressed file size
    ISS.read( reinterpret_cast<char *>(&m_NumBlocks), 4 );			// number of blocks
	...
	{
        ISS.read( reinterpret_cast<char *>(&m_War3Identifier), 4 );	// version identifier
        ISS.read( reinterpret_cast<char *>(&m_War3Version), 4 );		// version number
	}

    ISS.read( reinterpret_cast<char *>(&m_BuildNumber), 2 );			// build number
    ISS.read( reinterpret_cast<char *>(&m_Flags), 2 );				// flags
    ISS.read( reinterpret_cast<char *>(&m_ReplayLength), 4 );			// replay length
	...
	for( uint32_t i = 0; i < m_NumBlocks; i++ )
	{
        ...
        ISS.read( reinterpret_cast<char *>(&BlockCompressed), 2 );	// block compressed size
        ISS.read( reinterpret_cast<char *>(&BlockDecompressed), 2 );	// block decompressed size
        ...
        ISS.read( reinterpret_cast<char *>(CompressedData), BlockCompressed );
        ...
        if( BlockDecompressedLong != static_cast<uLongf>(BlockDecompressed) )
		{
        ...
        m_Decompressed += string( reinterpret_cast<char *>(DecompressedData), BlockDecompressedLong );
		delete[] DecompressedData;
		delete[] CompressedData;
...
void CPacked::Compress( bool TFT )
{
	...
	while( Position < Padded.size( ) )
	{
        ...
        int Result = compress( CompressedData, &BlockCompressedLong, reinterpret_cast<const Bytef *>(Padded.c_str( )) + Position, 8192 );
        ...
        CompressedBlocks.push_back( string( reinterpret_cast<char *>(CompressedData), BlockCompressedLong ) );
    ...
    UTIL_AppendByteArray( Header, static_cast<uint32_t>(m_Decompressed.size( )), false );
    UTIL_AppendByteArray( Header, static_cast<uint32_t>(CompressedBlocks.size( )), false );
    ...
    UTIL_AppendByteArray( Header, static_cast<uint32_t>(0), false );

	// calculate header CRC

	string HeaderString = string( Header.begin( ), Header.end( ) );
    uint32_t CRC = m_CRC->FullCRC( reinterpret_cast<unsigned char *>(const_cast<char *>(HeaderString.c_str( ))), HeaderString.size( ) );
	...
	for( vector<string> ::iterator i = CompressedBlocks.begin( ); i != CompressedBlocks.end( ); i++ )
	{
        ...
        UTIL_AppendByteArray( BlockHeader, static_cast<uint16_t>(( *i ).size( )), false );
        UTIL_AppendByteArray( BlockHeader, static_cast<uint16_t>(8192), false );

		// append zero block header CRC

        UTIL_AppendByteArray( BlockHeader, static_cast<uint32_t>(0), false );

		// calculate block header CRC

		string BlockHeaderString = string( BlockHeader.begin( ), BlockHeader.end( ) );
        uint32_t CRC1 = m_CRC->FullCRC( reinterpret_cast<unsigned char *>(const_cast<char *>(BlockHeaderString.c_str( ))), BlockHeaderString.size( ) );
		CRC1 = CRC1 ^ ( CRC1 >> 16 );
        uint32_t CRC2 = m_CRC->FullCRC( reinterpret_cast<unsigned char *>(const_cast<char *>(( *i ).c_str( ))), ( *i ).size( ) );
		CRC2 = CRC2 ^ ( CRC2 >> 16 );
